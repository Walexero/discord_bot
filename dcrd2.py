from ast import Str
from discord_bot.final_file.verify import API
import os
...
def _get_subscription_info():
    ...
    def _query_api(input_query):
        ...
        def _get_total_pages_from_headers():
            ...
            return lookup_get_header.headers["X-Wp-TotalPages"]
            
        _get_total_pages_from_headers()

        def _lookup_per_page(pages):
            for page_no in pages:
                lookup_page = wcapi.get(input_query,params={
                               "per_page": 10,
                               "page": page_no,
                               "role": "all",
                               "order_by": "id"
